...
- `tests/` — тесты основных модулей.
- `scripts/` — вспомогательные bash-скрипты для сборки и запуска.
- `third_party/` — внешние зависимости, если потребуется хранить локально.
...
### `include/postprocess/` и `src/postprocess/`

Постобработка результатов.

- `metrics.hpp` / `metrics.cpp` — расчёт метрик: максимум давления, минимум радиуса, максимум температуры и т.д.
- `focus_region.hpp` / `focus_region.cpp` — выделение области фокусировки акустического поля.
- `compare_runs.hpp` / `compare_runs.cpp` — сравнение нескольких запусков и геометрий.
...
Минимальные тесты по модулям.

- `test_config.cpp` — проверка чтения конфигурации.
- `test_geometry.cpp` — проверка построения геометрии.
- `test_acoustics.cpp` — проверка акустического решателя.
- `test_bubble.cpp` — проверка модели пузырька.
- `test_pipeline.cpp` — проверка полной связки.
...
- `run_pipeline.sh` — запуск основного сценария.
...
5. В `postprocess/` и `io/` сохраняются и анализируются результаты.
