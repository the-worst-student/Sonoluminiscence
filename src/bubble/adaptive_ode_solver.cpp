...
KmBubbleState AddScaledState(const KmBubbleState& state, const KmBubbleStateDerivative& derivative, double scale) {
    KmBubbleState result;
...
KmBubbleState AddWeightedDerivatives(const KmBubbleState& state,
                                   const KmBubbleStateDerivative& k1, double b1,
                                   const KmBubbleStateDerivative& k2, double b2,
                                   const KmBubbleStateDerivative& k3, double b3,
                                   const KmBubbleStateDerivative& k4, double b4,
                                   const KmBubbleStateDerivative& k5, double b5,
                                   const KmBubbleStateDerivative& k6, double b6,
                                   double dt) {
    KmBubbleState result;
...
struct Rk45StepResult {
    KmBubbleState HighOrderState;
    KmBubbleState LowOrderState;
...
double EstimateErrorNorm(const KmBubbleState& previous_state,
                         const KmBubbleState& high_state,
                         const KmBubbleState& low_state,
...
Rk45StepResult MakeRk45Step(const KmBubbleState& state, double t, double dt,
                            const AdaptiveOdeOptions& options,
                            const std::function<KmBubbleStateDerivative(double, const KmBubbleState&)>& rhs) {
    const KmBubbleStateDerivative k1 = rhs(t, state);

    const KmBubbleState y2 = AddScaledState(state, k1, dt * 1.0 / 5.0);
    const KmBubbleStateDerivative k2 = rhs(t + dt * 1.0 / 5.0, y2);

    const KmBubbleState y3 = AddWeightedDerivatives(state, k1, 3.0 / 40.0, k2, 9.0 / 40.0,
                                                  k1, 0.0, k1, 0.0, k1, 0.0, k1, 0.0, dt);
    const KmBubbleStateDerivative k3 = rhs(t + dt * 3.0 / 10.0, y3);

    const KmBubbleState y4 = AddWeightedDerivatives(state, k1, 3.0 / 10.0, k2, -9.0 / 10.0,
                                                  k3, 6.0 / 5.0, k1, 0.0, k1, 0.0, k1, 0.0, dt);
    const KmBubbleStateDerivative k4 = rhs(t + dt * 3.0 / 5.0, y4);

    const KmBubbleState y5 = AddWeightedDerivatives(state, k1, -11.0 / 54.0, k2, 5.0 / 2.0,
                                                  k3, -70.0 / 27.0, k4, 35.0 / 27.0,
                                                  k1, 0.0, k1, 0.0, dt);
    const KmBubbleStateDerivative k5 = rhs(t + dt, y5);

    const KmBubbleState y6 = AddWeightedDerivatives(state, k1, 1631.0 / 55296.0, k2, 175.0 / 512.0,
                                                  k3, 575.0 / 13824.0, k4, 44275.0 / 110592.0,
                                                  k5, 253.0 / 4096.0, k1, 0.0, dt);
    const KmBubbleStateDerivative k6 = rhs(t + dt * 7.0 / 8.0, y6);

    const KmBubbleState high_state = AddWeightedDerivatives(state, k1, 37.0 / 378.0, k2, 0.0,
                                                          k3, 250.0 / 621.0, k4, 125.0 / 594.0,
                                                          k5, 0.0, k6, 512.0 / 1771.0, dt);

    const KmBubbleState low_state = AddWeightedDerivatives(state, k1, 2825.0 / 27648.0, k2, 0.0,
...
AdaptiveOdeResult AdaptiveOdeSolver::Solve(const KmBubbleState& initial_state, double t_start, double t_end, const AdaptiveOdeOptions& options,
    ...
    KmBubbleState state = initial_state;

    result.Samples.push_back(KmBubbleSample{t, state});
