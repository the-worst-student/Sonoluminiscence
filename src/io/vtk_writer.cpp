...
void VtkWriter::WriteAxisymmetricSlice(
    ...
    const std::size_t BubbleNodeIndex =
        FindClosestNodeIndex(field_data.Nodes, bubble_r, bubble_z);
    ...
    WritePointScalar("grad_p_r_re_pa_per_m", [](const AcousticNodalField& Field) { return Field.GradPressureRRealPaPerM; });
    WritePointScalar("grad_p_r_im_pa_per_m", [](const AcousticNodalField& Field) { return Field.GradPressureRImagPaPerM; });
    WritePointScalar("grad_p_z_re_pa_per_m", [](const AcousticNodalField& Field) { return Field.GradPressureZRealPaPerM; });
    WritePointScalar("grad_p_z_im_pa_per_m", [](const AcousticNodalField& Field) { return Field.GradPressureZImagPaPerM; });
    WritePointScalar("grad_p_abs_pa_per_m", [](const AcousticNodalField& Field) { return Field.GradPressureAbsPaPerM; });
    ...
    auto WriteCellScalar = [&](const std::string& name, auto getter) {
        Output << "SCALARS " << name << " double 1\n";
        Output << "LOOKUP_TABLE default\n";
    ...
    WriteCellScalar("pressure_abs_cell_pa", [](const AcousticCellField& Field) { return Field.PressureAbsPa; });
    WriteCellScalar("grad_p_abs_cell_pa_per_m", [](const AcousticCellField& Field) { return Field.GradPressureAbsPaPerM; });
...
void VtkWriter::WritePseudoVolume(
    ...
    const std::size_t BubbleNodeIndex2D =
        FindClosestNodeIndex(field_data.Nodes, bubble_r, bubble_z);
    const std::size_t PointCount =
        NodeCount2D * static_cast<std::size_t>(sectors) + 1;
    const std::size_t CellCount =
        CellCount2D * static_cast<std::size_t>(sectors) + 1;
    ...
    for (int Sector = 0; Sector < sectors; ++Sector) {
        const double Phi =
            2.0 * cPi * static_cast<double>(Sector) / static_cast<double>(sectors);
    ...
    for (int Sector = 0; Sector < sectors; ++Sector) {
        ...
        for (const TriangleElement& Element : field_data.Elements) {
            const std::size_t P0 = PointIndex(
                static_cast<std::size_t>(Sector),
                static_cast<std::size_t>(Element.Node0),
                NodeCount2D);
            const std::size_t P1 = PointIndex(
                static_cast<std::size_t>(Sector),
                static_cast<std::size_t>(Element.Node1),
                NodeCount2D);
            const std::size_t P2 = PointIndex(
                static_cast<std::size_t>(Sector),
                static_cast<std::size_t>(Element.Node2),
                NodeCount2D);
            const std::size_t P3 = PointIndex(
                static_cast<std::size_t>(NextSector),
                static_cast<std::size_t>(Element.Node0),
                NodeCount2D);
            const std::size_t P4 = PointIndex(
                static_cast<std::size_t>(NextSector),
                static_cast<std::size_t>(Element.Node1),
                NodeCount2D);
            const std::size_t P5 = PointIndex(
                static_cast<std::size_t>(NextSector),
                static_cast<std::size_t>(Element.Node2),
                NodeCount2D);
