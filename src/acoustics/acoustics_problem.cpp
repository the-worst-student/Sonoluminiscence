...
AcousticsProblem AcousticsProblem::FromConfig(const ProjectConfig& config){
    ...
    problem.Omega = 2.0 * M_PI * problem.FrequencyHz;
    problem.WaveNumber = problem.Omega / problem.SoundSpeed;

    const double source_flux_imag = problem.Omega * problem.SourceNormalVelocity * problem.Density;
...
void AcousticsProblem::Validate() const {
    ...
    if (std::find(RigidBoundaryTags.begin(),RigidBoundaryTags.end(), SourceBoundaryTag) != RigidBoundaryTags.end()) {
        throw std::invalid_argument("Source boundary tag must not belong to rigid boundaries");
    }
}
